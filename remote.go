...
func (elem *remoteWE) FindElement(by, value string) (WebElement, error) {
	urlTemplate := fmt.Sprintf("/session/%%s/element/%s/element", elem.id)
	response, err := elem.parent.find(by, value, "", urlTemplate)
	if err != nil {
		return nil, err
	}
...
func (elem *remoteWE) FindElements(by, value string) ([]WebElement, error) {
	urlTemplate := fmt.Sprintf("/session/%%s/element/%s/element", elem.id)
	response, err := elem.parent.find(by, value, "s", urlTemplate)
	if err != nil {
		return nil, err
	}
...
func (elem *remoteWE) location(suffix string) (*Point, error) {
	if !elem.parent.w3cCompatible {
		wd := elem.parent
		urlTemplate := "/session/%s/element/%s/location" + suffix
		requestURL := wd.requestURL(urlTemplate, wd.id, elem.id)
		response, err := wd.execute("GET", requestURL, nil)
		if err != nil {
			return nil, err
		}
